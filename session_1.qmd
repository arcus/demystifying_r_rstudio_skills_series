...
-   Slides with animated gifs may need to be "reloaded" in the browser to run
...
This first slide is just to help you navigate this slide deck if you look at these slides on your own!
:::
...
Hi everyone, and welcome to our session today!  I'd like to begin by briefly introducing myself and introducing Arcus, for those of you who may not be familiar with us.  My name is ______ (add pronouns here if you want), and I'll be leading today's session.

Arcus is an initiative by the Research Institute aimed at promoting data discovery and reuse and increasing research reproducibility throughout the research lifecycle, as you can see in the graphic on the right.  [Pause]
...
You can also, without restriction or any prerequisites, look at our Arcus Sharepoint site, which has a calendar of events and lots more information about Arcus.

A little more about me: I work in Arcus Education.  Our role is to support CHOP scientists by helping researchers acquire data science skills.  That's why we have Skills Series like this one!
...
Please check out that page and find out more about some of our services.  You can see I've added a screenshot of that page to the slide here on the left side.  You can also email us at arcus dash education at chop dot edu.
:::
...
One of our methods is the Skills Series, and that's what you're in today.

In this series, we're going to take just two sessions to help demystify R and RStudio.  If you have heard people talk about these concepts and you have only a vague idea what they are, and want to know more, without having to actually download anything or write code, this is the right series for you. 

After you finish these two sessions, you'll understand what R is, what RStudio is, why people use these tools, and next steps for learning to use them, if you want.
...
::: notes
So let's get started with this first session. In this session we're going to cover an introduction to R, RStudio (aka Posit), and R Markdown / Quarto.  
...
::: notes
OK, if you've never seen any R language before, here's an example.  This is real R code that we use in my team.  This is just to show you what R looks like, so you get an idea.  Don't worry, there won't be a quiz!
...
First, we're ingesting some data from REDCap.  It's a survey we give related to a webinar we give twice a month, called Arcus 101.  We want to measure our effectiveness, and we analyze this survey data using R.

So we ingest the data,

In line 9, we get rid of a value we don't need.

Then we do some transformation.  We want to make sure R knows the proper order of our Likert scale values.  We have a set of values that has to do with knowledge, a set of values that has to do with opinion before our talk, and a set of values that has to do with opinion after our talk.

In lines 36 and 37, we do a little math to measure change.

Then we make a data visualization in lines 42 through 46, and in line 50, we save this as a file we can use later.
...
However, if you're going to need to repeat your analysis over and over again, or have someone else do it, think about the steps you take.  Will it be hard to describe exactly what cell to put data in, and how to click and drag the values in just the right way?  If there are lots of steps and doing things manually is error prone, hard to describe, or just plain annoying, you might be better off writing a script.
...
In science, we've been hearing a lot lately about the "reproducibility crisis".
...
::: {.tiny-text}
Credit: "The Difference", XKCD, CC A-NC 2.5

Joy waiting on permission for cartoon permission for cartoon from https://www.himelblau.com/cartoons/science-and-the-lab... in the meantime...
:::

::: 
::: {.column .tiny-text width="30%"}
...
That's why having an automated script, which handles all of the steps without human intervention, can be really very helpful.  And when we're in an economy where we have to do more with less, doing it for free, without an expensive license, is really useful, especially if you want to share your findings with colleagues in lower-resourced settings.
...
Then, there's **RStudio**. RStudio was the original name of a company as well as the name of a piece of software the company makes (and makes available in both free and paid versions). A couple of years ago, the company changed its name from RStudio to Posit, but they kept the old name on their software.
...
::: notes
OK, so here I'm going to run code on the left and it will be executed on the right.  You can execute code using a keyboard shortcut or by using the dropdown menu.  You can see that graphics appear in a separate menu.  And I'm creating a few different graphics.  If I want to see the folder that contains them all, I have to open a new program.  I use a Mac, so in my case that's Finder.  Finally, I can use the Help function, and a separate window opens up.  That's the R app.  It's not terrible, but it's not great, either.
...
We can see the plots in a couple of different places in RStudio, depending on our preferences, and we can see our files listed out right in RStudio, and click on our files to open them and see them in a file viewer.

Help is also located in a pane in the same window, which can be helpful.

I can also take a look at the data I've ingested and get an idea of what my data looks like, and I can also create a report by "rendering" my code and making an HTML report that includes my code, some headers I wrote that describe my code, and shows the graphs I made.  This kind of report creation is a big reason why people use RStudio.  RStudio supports the creation of dashboards and reports that help you not only analyze your data, but communicate what you've discovered to different kinds of audiences effectively.  If you want to learn more about that, come to the next talk in this series, called literate statistical programming.
...
**Posit Workbench** -- a version of the RStudio IDE, also now known as Posit, that can be accessed from a web browser. It's hosted on a server, such as on a server your department runs that everyone in your team can access. 

**RStudio Desktop** -- a version of the RStudio IDE that is installed on a personal device like your desktop or laptop. This is what many people at CHOP use, installing it on their CHOP devices or using it on a virtual device like a VDI. 

It's important to note that regardless of which version of RStudio that you use, you will be accessing the same general product.  All of these pretty much look and act the same.  So let's get into what you might need to do at CHOP if you want to work with R.
...
First, a few caveats to keep in mind.  At CHOP, we work with sensitive data, whether that's health care data, research data, or trade secrets about our enterprise.  We want to be careful.  Lots of people do work with R and RStudio at CHOP, and you'll almost certainly need to put in a ticket to get the right software installed.  At this point, if you're not already following along in the slides, I'm going to really encourage you to grab this slide link because this slide and the next will have important information that will be useful for you when you decide to work with R and RStudio.
...
::: notes
If you're a researcher at CHOP, there are additional places you might want to use R.  And keep in mind, I work on the research side, so this is what I know best.  If you're on the clinical or operational side of CHOP and there's something about working with R and RStudio on your side that I've omitted, please let me know so I can update the slides!
...
::: notes
We're a group that likes to measure our effectiveness and make changes based on what we learn.  So before we do questions and answers, we like to make sure to give our learners a poll to see if we made a good use of your time.  We're going to ask if we were effective in reaching our goals today, and those goals are on the screen in order to help you answer that question.  Once you answer that poll, feel free to type a question into chat or come off mute and ask to clarify anything I can explain a bit better.
